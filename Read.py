...
class Read:
    def __init__(self):
        ...
        self.url = 'https://www.goodreads.com/review/list/46998003.xml?key={}&v=2&shelf=read'.format(self.api_key)

    def get(self):
        request = requests.get(self.url)

        dict_read = xmltodict.parse(request.text)

        return dict_read

    def add(self, id):
        url = 'https://www.goodreads.com/shelf/add_to_shelf.xml?name=read&book_id={}'.format(id)
        requests.post(url)

    def remove(self, id):
        url='https://www.goodreads.com/shelf/add_to_shelf.xml?name=read&book_id={}?a=remove'.format(id)
        requests.post(url)

    def show(self, dict_read, frame):
        i = 0
        ...
        for book in dict_read['GoodreadsResponse']['reviews']['review']:
            if i <= 10:
